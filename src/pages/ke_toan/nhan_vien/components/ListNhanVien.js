...
import { GetNhanVienRequest, exportNVRequest } from '../../../../reducers/ke-toan/NhanVien/NhanVienAction';
...
export default function ListNhanVien() {
  ...
  */
  ...
  useEffect(() => {
    dispatch(GetNhanVienRequest());
  }, []); 

  return (
    <>
      <MaterialTable style={{ width: '100%' }}
        title='Danh sách kế toán nhân viên'
        columns={[
          {
          ...
          {
            title: 'Bậc', field: 'bac', align: 'center', cellStyle: { padding: '5px' },
            render: rowdata => <div style={{ width: 120 }}>{rowdata.bac}</div>,
          },
          {
          ...
          },
          {
            ...
            render: rowdata => <Link to={`/ke_toan/sua_thong_tin_nhan_vien_kt/${rowdata.id}`}><button type="button" className='btn btn-primary px-3' style={{ borderRadius: "0px" }}>
